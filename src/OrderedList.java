	...
	public void add(Node toInsert) {
		if(toInsert == null) {
			...
			if(top == null) {
				...
				Node curr = top;
				while(curr.getNext() != null && curr.getData().getName().compareTo(toInsert.getData().getName()) <= 0) {
					curr = curr.getNext();
				}
			    if (curr == top && curr.getData().getName().compareTo(toInsert.getData().getName()) > 0) {
			        toInsert.setNext(top);
			        top = toInsert;
			    }
			    else {
